...
1. **Frontend registries** in `frontend/src/lib/*.ts` (`shopAllItems.ts`, `navigation.ts`) — drive most of what public visitors see today. Articles, Retreats, Programs, Shop Bridge Pages (metadata + the `/shop/[slug]` product lists), and `/shop` (Shop-All grid) are already migrated to backend-fed reads via `publicBackendJson`; see Decisions #16–#21. Remaining `shopProducts` consumers (home featured sets, search overlay, cart, checkout, Lifestyle Gift Pouch option lists, seasonal drops, HowSeijakuWorks) still read the registry until Phase 4b.iii+.
