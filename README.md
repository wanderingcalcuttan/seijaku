...
- **Curated bridge clients (Decision #30) and editorial bridge pages (Decision #31).** `/shop/diffusers`, `/shop/lifestyle`, `/a-seijaku-life`, and the home `JournalPreviewSection` use small hardcoded slug constants in their page-client files to control which records surface (the brand has no curation UI yet). Three new `ShopBridgePage` records — `home`, `our-story`, `seasonaldrops-hemanta` — let admins upload editorial media (hero images, card images, video loops, image breaks) for those non-shop routes via the existing `/admin/bridge-pages` form. See `CONTENT_MODEL.md` "Bridge-Page Rendering Rules" + "Editorial Bridge Pages" for the full per-route table.
- **`/a-seijaku-life` and home `JournalPreviewSection` (Decision #32) are curated to three articles.** The other published articles in `/admin/articles` remain reachable by direct URL but do not surface in public discovery.
