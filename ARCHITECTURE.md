...
- `/shop` (Shop-All canonical route + the `/shop-all` redirect) — reads via `publicBackendJson("/catalog/products", { tags: ["products"] })`; see Decision #21.
- home `RitualSetsSection`, global `SearchOverlay`, and `howSeijakuWorks.options.ts` — the first two lazy-fetch the public catalog client-side via `/api/public/catalog/products(/:slug)` on mount / first open; the third is pure static data with hand-curated image paths (Decision #22).
- Live Calm Gift Pouch picker options on `/shop/lifestyle` — derived at render time from the `ProductView[]` prop already flowing into `LifestylePageClient` via `buildLifestyleSections(products)` in `lifestyleSetConfig.ts` (Decision #23).
- `/seasonaldrops-hemanta` Reserve-button drawers — server component `page.tsx` fetches the four `hemanta-*` slugs via `fetchProductBySlug` and passes a `Record<slug, ProductView>` prop down (Decision #24).
- `/checkout`, `/collection`, and the four bridge-page client drawers — client-side lazy fetches against `/api/public/catalog/products/:slug` for checkout/collection; the bridge-page drawers reuse the `ProductView[]` prop already flowing into them (Decision #25).

As of Phase 4b.final (Decision #26), `frontend/src/lib/shopAllItems.ts` is deleted. The frontend library layer now holds only structural + editorial concerns: `shop-routes.ts` (canonical route map), `shop-taxonomy.ts` (filter unions, matchers, release-date lookup), `product-types.ts` (ProductView shape, status helpers, fetchers), `story-types.ts` (Story shape + fetcher, Decision #29), and the domain-specific normalizer modules (`bridge-page-types.ts`, `retreat-types.ts`, `program-types.ts`, `seijaku-life-types.ts`). The admin "Sync New Products" tool and its backend `/admin/products/sync-new` endpoint are deleted.
