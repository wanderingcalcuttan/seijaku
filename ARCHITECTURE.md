...
- Live Calm Gift Pouch picker options on `/shop/lifestyle` — derived at render time from the `ProductView[]` prop already flowing into `LifestylePageClient` via `buildLifestyleSections(products)` in `lifestyleSetConfig.ts` (Decision #23). Remaining `shopProducts` consumers (cart, checkout, `/collection`, `/seasonaldrops-hemanta`) migrate in Phase 4b.v onwards.
