...
- `/checkout`, `/collection`, and the four bridge-page client drawers — client-side lazy fetches against `/api/public/catalog/products/:slug` for checkout/collection; the bridge-page drawers reuse the `ProductView[]` prop already flowing into them (Decision #25). After this phase only `SyncRegistryButton.tsx` (admin tool) references `shopProducts` — deleted in Phase 4b.final.
