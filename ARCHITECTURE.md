...
As of Phase 4b.final (Decision #26), `frontend/src/lib/shopAllItems.ts` is deleted. The frontend library layer now holds only structural + editorial concerns: `shop-routes.ts` (canonical route map), `shop-taxonomy.ts` (filter unions, matchers, release-date lookup), `product-types.ts` (ProductView shape, status helpers, fetchers), and the domain-specific normalizer modules (`bridge-page-types.ts`, `retreat-types.ts`, `program-types.ts`, `seijaku-life-types.ts`). The admin "Sync New Products" tool and its backend `/admin/products/sync-new` endpoint are deleted.
